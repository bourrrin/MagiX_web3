...
function loadShips() {
    for (let i = 0; i < 3; i++) {
        for (let j = 0; j < 18; j++) {
            let source;
            if (j == 17 && i == 2) source = "img/ships/easter_egg.png";
            else source = "img/ships/ship" + j + ".png";
...
function animation_login() {
    ...
    creatFlashElement();
    setTimeout(creatFlashElement, 200);
}
//#endregion
    ...
    fetch("loginAjax.php", {
        ...
        .then((response) => {
            ...
            if (response == true) {
                ...
                document.querySelector(".login").classList.add("login_erreur");
                setTimeout(() => {
                    document.querySelector(".login").classList.remove("login_erreur");
                    setTimeout(() => {
                        document.querySelector("#message_erreur").innerHTML =
                            "Username or Password Invalid";
                        document.querySelector("#login_name").value = "";
